...
import { LOCALES, DEFAULT_LOCALE, SITE_URL, stripLocalePrefix, withLocalePrefix, useI18n, BCP47 } from "@/lib/i18n";
...
export function Seo({ title, description, canonical, ogType = "website", jsonLd, children }: SeoProps) {
  ...
  // trust it; otherwise derive from the current path in the active locale.
  const canonicalUrl =
    canonical ?? `${SITE_URL}${withLocalePrefix(basePath, lang)}`;
